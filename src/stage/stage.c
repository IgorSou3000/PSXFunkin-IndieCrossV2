...
static void Stage_ScrollCamera(void)
{
	...
	if (!(stage.flag & STAGE_FLAG_PAUSED))
	{
			...
			stage.camera.x += FIXED_MUL(stage.camera.tx - stage.camera.x, stage.camera.td);
			stage.camera.y += FIXED_MUL(stage.camera.ty - stage.camera.y, stage.camera.td);
			stage.camera.zoom += FIXED_MUL(stage.camera.tz - stage.camera.zoom, stage.camera.td);
		#endif
...
//Note stuff
static void Stage_SpecialNotes_Check(Note* note)
{
	if (note->type & NOTE_FLAG_PARRY)
		Audio_PlaySFX(stage.sounds[1], 70);
}
//Note hit detection
...
static void Stage_TimerDraw(void)
{
	...
	RECT_FIXED bar_dst = {FIXED_DEC(-100,1), FIXED_DEC(-112,1), 0, FIXED_DEC(5,1)};

	if (stage.save.downscroll)
		bar_dst.y = -bar_dst.y - bar_dst.h;

	u8 r = ((stage.opponent->health_b >> 16)  & 0xFF) / 2;
	u8 g = ((stage.opponent->health_b >>  8)  & 0xFF) / 2;
	u8 b = ((stage.opponent->health_b >>  0)  & 0xFF) / 2;

	//Display timer
	if (stage.save.showtimer)
	{
		stage.font_cdr.draw(&stage.font_cdr,
			stage.stage_def->song_name,
			0, 
			(stage.save.downscroll) ? SCREEN_HEIGHT2 - 8 - 9 : -SCREEN_HEIGHT2 + 6,
			FontAlign_Center
		);

		bar_dst.w = bar_fill.w << FIXED_SHIFT;
		Stage_DrawTexCol(&stage.tex_hud1, &bar_fill, &bar_dst, stage.bump, r, g, b);
...
static void Stage_DrawPause(void)
{
  ...
  stage.pause_scroll += FIXED_MUL(next_scroll - stage.pause_scroll, FIXED_DEC(1,1) / 8);
...
static void Stage_DrawNotes(void)
{
	...
	for (Note *note = stage.main_chart.cur_note; note->pos != 0xFFFF; note++)
	{
		...
		else
		{
			...
			else if (note->type & NOTE_FLAG_PARRY)
			{
...
void Stage_Load(StageId id, StageDiff difficulty, boolean story)
{
	...
	//Load HUD textures
	Gfx_LoadTex(&stage.tex_hud0, IO_Read("\\HUD\\HUD0CUP.TIM;1"), GFX_LOADTEX_FREE);
	...
	stage.character_bump = stage.defaultzoom;
	...
	switch (stage.state)
	{
		...
		{		
  		...
  		Events_StartEvents();
			...
			{
				//New camera system based in psych engine camera
				fixed_t larpval = FIXED_DEC(1,1) - FIXED_MUL(timer_dt, FIXED_DEC(3125,1000));

				stage.bump 		 			 = LERP(FIXED_UNIT, 			 stage.bump, 		 			 larpval, FIXED_UNIT);
				stage.sbump 			   = LERP(FIXED_UNIT, 			 stage.sbump, 				 larpval, FIXED_UNIT);
				stage.character_bump = LERP(stage.defaultzoom, stage.character_bump, larpval, FIXED_UNIT);
